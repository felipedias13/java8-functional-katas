...
public class Kata10 {
    public static List<Map> execute() {
        ...
        return lists.stream().map(mapToImmutableMap(videos)).collect(Collectors.toList());
    }
    
    public static Function<Map, ImmutableMap<String, Object>> mapToImmutableMap(List<Map> videos){
    	return list -> ImmutableMap.of("name", list.get("name"), "videos",
    			videos.stream().filter(video -> video.get("listId").equals(list.get("id")))
    			.map(video -> ImmutableMap.of("id", video.get("id"), "title", video.get("title"))).collect(Collectors.toList()));
    }
}
